# This script makes maps of global SO2 with center longitude of 180
...
def plot_map(data, lat, lon,extents, label, ax, diff_cmap=False):
    ...
    if diff_cmap: 
        mesh = ax.pcolormesh(lon_grid, lat_grid, regional_data, transform=ccrs.PlateCarree(central_longitude=180), cmap='RdBu_r', vmin=-0.2, vmax=0.2)
    else:
        mesh = ax.pcolormesh(lon_grid, lat_grid, regional_data, transform=ccrs.PlateCarree(central_longitude=180), cmap='YlOrBr', vmin=0, vmax=0.5)
